...
int main()
{
	...
	*D = 2;
	...
	Dmobj = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int), NULL, &ret);
	...
	ret = clEnqueueWriteBuffer(command_queue, Dmobj, CL_TRUE, 0, sizeof(int), D, 0, NULL, NULL);
	...
	ret = clSetKernelArg(kernel[1], 3, sizeof(cl_mem), (void*)&Dmobj);

	printf("CL : Task parallel\n");
	//커널 실행 부분 - 데이터 병렬 실행
	for(i=0;i<4;i++){
