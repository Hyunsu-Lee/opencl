...
if(!fp){
	...
	Admobj = clCreateBuffer(context, CL_MEM_READ_WRITE, 4*4*sizeof(float), NULL, &ret);
	Bdmobj = clCreateBuffer(context, CL_MEM_READ_WRITE, 4*4*sizeof(float), NULL, &ret);
	Cdmobj = clCreateBuffer(context, CL_MEM_READ_WRITE, 4*4*sizeof(float), NULL, &ret);
	...
	ret = clEnqueueWriteBuffer(command_queue, Admobj, CL_TRUE, 0, 4*4*sizeof(float), B, 0, NULL, NULL);
	ret = clEnqueueWriteBuffer(command_queue, Bdmobj, CL_TRUE, 0, 4*4*sizeof(float), B, 0, NULL, NULL);
	...
	for(i = 0;i<3;i++){
		...
		ret = clSetKernelArg(kernel[3], 0, sizeof(cl_mem), (void*)&Admobj);
		ret = clSetKernelArg(kernel[3], 1, sizeof(cl_mem), (void*)&Bdmobj);
		ret = clSetKernelArg(kernel[3], 2, sizeof(cl_mem), (void*)&Cdmobj);
